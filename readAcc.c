 ...
 *
 * readAcc.c
 * Includes functions to init accelerometer
 * and read values
 *
 *
 *    C. P. Moore
...
void updateRefPos(circBuf_t* x, circBuf_t* y,circBuf_t* z){
    ...
    for (i = 0; i<BUF_SIZE; i++){
       x_sum = x_sum + readCircBuf(x);
        z_sum = z_sum + readCircBuf(y);
        y_sum = y_sum + readCircBuf(z);

    }
    zeroPos.x = (2 * x_sum + BUF_SIZE) / 2 / BUF_SIZE;
    zeroPos.y = (2 * y_sum + BUF_SIZE) / 2 / BUF_SIZE;
    zeroPos.z = (2 * z_sum + BUF_SIZE) / 2 / BUF_SIZE;
